...
def encrypt_message(key: str, message: str) -> str:
    ...
    # Create columns based on the key length
    ciphertext_cols = [""] * len(key)
    
    # Fill the columns character by character
    ...
    # Determine the order to read columns out based on the key
    key_order = sorted([(char, i) for i, char in enumerate(key)])
    
    # Read the columns in the sorted key order
...
def decrypt_message(key: str, message: str) -> str:
    ...
    # The last `num_shaded` columns in the original order will be one character shorter
    ...
    # Read the ciphertext into columns based on the sorted key order
    pointer = 0
